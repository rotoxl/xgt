...
var dif=6
function sacaW(){
	//~ var bd=$('#botoneraDisenho')
	return $('body').innerWidth()- (disenhador && disenhador.modoDisenho? $('#botoneraDisenho').outerWidth():0) +dif
	}
function sacaH(){
...
function Explorador(aplicacion, icoWeb, icoGotta, acceptCharset){
	...
	this.dispositivoMovil=navigator.userAgent.containsIgnoreCase('mobile')
...
Explorador.prototype.onResize=function(inicializar){
	...
	if (inicializar==true || splitterV==null){
		...
		if (mlActual && mlActual.tipoPanel1==0 && (!disenhador || !disenhador.modoDisenho))
...
Explorador.prototype.inicializa=function(estadoDisenho, estadoDepuracion, ml){
	...
	if('Disenhador' in window)
		disenhador=new Disenhador()
	if (estadoDepuracion) 
		activarDepuracion()
...
Explorador.prototype.cargaVistas=function(mlInicial){
	loadJSONDoc ( 'json', { 'accion':'vistasEXP', aplicacion:this.aplicacion},
				function(req) {
					...
					if (!x.menu){
						x={menu:{
							filas:[	[null,'No hay datos o no existe el nivel menuGeneral',null,'1','menuVer','0','carpeta','','','','','','','v','1',null,'1','menuglobal']],
...
Explorador.prototype.montaMenu=function(filas, claseAdicional){//N menus desplegables, con lanzamiento de trámites etc. 
	...
	for (var i=0; i<filas.filas.length; i++){	
		...
		if (cd_menupadre=='nivel0' || cd_menupadre=='0'){
			...
			if (ds.startsWith('-')) {
				ds=ds.substring(1)
				var hijo=creaObjProp('a', {id:idMenu, title:titulo, className:'menuML separador '+claseAdicional, texto: ds})
				listaHijos=envuelve(hijo)
				}
			else 	{
				xul[idxul]=jQuery(creaObjProp('ul', {id:'submenu'+cd_boton, className:claseAdicional}))
				listaHijos=[creaObjProp('a', {id:idMenu, title:titulo, className:'menuML droprightmenu '+claseAdicional, texto: ds, 'style.backgroundImage':completaRutaImg(imagen, true)}), xul[idxul][0] ]
				}
			...
			if (ds.startsWith('-')) {
				ds=ds.substring(1)
				enlace=creaObjProp('a', {id:idMenu, title:titulo, className:'menuML separador '+claseAdicional, texto: ds})
				}
			else  {
				enlace=creaObjProp('a', {id:idMenu, title:titulo, className:'menuML '+claseAdicional, texto:ds, 'style.backgroundImage':completaRutaImg(imagen, true)})
				enlace=this.enchufaOnClick(enlace, cd, url, cd_camino, cd_tramite)
				}
	...
	loadJSONDoc ( 'json', { 'accion':'md', 'md':idMd, aplicacion:this.aplicacion},
			function(req) {
					...
					if (url) {
						...
						fnPostCarga()
					if (disenhador && disenhador.modoDisenho) disenhador.cambioMD(md, exp.getMD(exp.idMlActivo))
					}, 
			function()
...
//esta func es la que hay que redefinir en los js particulares de cada aplicación para esos malditos ajustes de internet explorer
//	La mejor manera que se me ocurre consiste en añadir clases especiales a los objetos conflictivos: 	
//	var versionIE=versionInternetExplorer()
//	if (versionIE > 0)
//		ponEstilo( control('botoneraIzq'), 'ie'+versionIE)
//
//	y luego en tu CSS ya defines
//		#botoneraIzq {definición normal, para todos los navegadores y versiones}
//		div.ie6#botoneraIzq {definición exclusiva para ie6}  	-> aquí lo recomendado es utilizar margin-left y margin-top para ajustar al pixel la posición
//		div.ie7#botoneraIzq {definición exclusiva para ie7}	-> aquí lo recomendado es utilizar margin-left y margin-top para ajustar al pixel la posición
//	De esta forma es sencillo tener un objeto con clases distintas según el navegador
}	
...
ModoDetalle.prototype.datosFRM=function(){
	return {numTramites:0, camino:0, tramite:'Propiedades del detalle', tipo:'frm',"titulo" : 'Propiedades del detalle "'+this.id+'"', controles:[
			{tipo : 'etiqueta','id' : 'sinnombre9782880035098282','maxlength' : 0,'valor' : '','bloqueado' : false,'validacion' : '','ds' : 'Origen de los datos'},
			{tipo:'bsc', id:'DIC_Tablas.Nombre', ds:'Tabla',controles:[
					{tipo: 'lvw',id:'nombre',maxlength : -25, valor:this.tb, ds:''}
					]},
...
ModoDetalle.prototype.cambiaModoDisenho=function(modoDisenho){
	...
	if (modoDisenho) {
		var altura=Math.max(getAltoPantalla()-$('#panelDer').offset().top, this.maxAltura)
		altura+=40 //dejamos un pequeño margen, viene bien para diseñar
		...
		this.div.style.height='0px' // Math.max(getAltoPantalla()-$('#panelDer').offset().top, this.maxAltura)+'px'
...
function _datoQueSeMuestra(datosFRM, ctl){
	if (!disenhador.modoDisenho)return false
...
Control.prototype.fnDatoQueSeMuestra=function(idControl) {
	return function(event){if (disenhador && disenhador.modoDisenho) return datoQueSeMuestra(idControl, event)}
}
Control.prototype.JSON=function(){
	...
	if (this.tc=='cmd')
		ret+=pareja('colores', colorVB(this.colorFondo)+' '+ colorVB(this.colorFrente))
	else if (['par', 'bsp', 'bsm', 'lbl', 'txt', 'exp'].contains(this.tc))
		ret+=pareja('colores', colorVB(this.colorFrente)+' '+ colorVB(this.colorFondo))
	else if (this.tc=='rd')
		ret+=pareja('colores', colorVB(this.colorFondo))
	else
		ret+=pareja('colores', '')
	
	if (disenhador.panelesInvertidos)
...
ControlPAR.prototype.toDOM = function() {
	...
	if (this.tipoDato=='date') {
		var a=generaBotonCalendario(false, idInput)
		a.tabIndex=-1
...
ControlPAR.prototype.aplicaEstilo=function(){
	if (this.claseCSS) {
		...
		try{
			...
			this.par.style.border='1px solid '+this.par.style.backgroundColor
			}
		catch(e){
			//console.log(e)
			}
...
ControlPAR.prototype.propsPorDefecto={ancho:1500, alto:255, izq:300, caption:''}
/////////////////////////////
	...
	return {numTramites: 0,camino: 0,tramite: 'Propiedades del botón', tipo: 'frm', controles:[
			...
			{tipo: 'etiqueta',id: 'sinnombre92z82','maxlength': 0,'valor': '','bloqueado': false,'validacion': '','ds': 'Trámite existente'},		
				{tipo: 'bsc',id:'TRA_TramitesObjetos.DS_Tramite', ds: 'Trámite',colsQueBloquea: 0,colsBloqueadas: 0,controles:[
						{tipo: 'lvwNumero',id:'cd_tramite',maxlength: 20,valor: this.tramite, ds: ''}
						]},
			{tipo: 'etiqueta', id: 'sinnombre9zz282','maxlength': 0,'valor': '','bloqueado': false,'validacion': '','ds': 'Trámite nuevo (se incluirá en TRA_TramitesObjetos y en TRA_Acciones)'},
				...
				{tipo: 'lvw', id:'DS_TramiteNuevo', maxlength:250,valor:'',obligatorio:false, ds: 'Descripción'},
			
			{tipo: 'etiqueta', id: 'sinnombre911zz282','maxlength': 0,'valor': '','bloqueado': false,'validacion': '','ds': 'Javascript'},
				{tipo: 'lvw', id: 'js','maxlength': -100,'valor': this.js,'bloqueado': false, 'ds': 'Código js', dsExtendida:'La función js deberá estar definida en "'+exp.aplicacion+'.js". Ejemplo: miFuncion'},
...
ControlLBL.prototype.toDOM = function() {
	...
	if (this.claseCSS) {
		...
		var contenedor=control(this.md.idx)
		if (contenedor && lbl.style.backgroundColor==contenedor.style.backgroundColor) {
			lbl.style.background='transparent'
			lbl.style.border='1px none' // ie6
			}
...
ControlIMG.prototype.datosFRM=function(){
	return {numTramites:0, camino:0, tramite:'Propiedades de la imagen', tipo:'frm', controles:[
			...
			{tipo: 'lvw', id:'alt', valor:this.alt, maxlength : -85, ds:'Descripción', dsExtendida:'Descripción del contenido de la imagen que será ofrecida a personas con discapacidad visual'},
...
ControlTAB.prototype.rellena=function(valores, dinamicos){
	...
	var modoDisenho=(disenhador && disenhador.modoDisenho)
...
ControlLVW.prototype.datosFRM=function(){
	...
	if (this.tc=='lvw') {
		ret.controles.push({tipo: 'lvw', id:'summary', maxlength : -85, valor:this.summary, ds:'Descripción', dsExtendida:'Descripción del contenido de la lista que será ofrecida a personas con discapacidad visual'})
...
ControlSUBGRID.prototype.expandir=function(xfila, xcolumnasOcultas, xtr) {
	...
	return function(event){
		...
		var subRejilla=new SubGrid(self.numControl+'_'+cd, nivel) 
...
ControlBSP.prototype.toDOM = function() {
	...
	if (bsp.childNodes.length) {
		txt=bsp.childNodes[1]
   		des=bsp.childNodes[3]
		}
	else{
		// añadimos botón
		var idBSP=this.id
		var self=this
		var cmd= creaObjProp('div', {className:'cmd_'+this.tc, 
							tabIndex:-1, 
							title:'[Click aquí para buscar]', 
							onclick:function(){abreVentanaBuscador(self, self.md); return false;},
							href:'#',
							texto:espacioDuro})
		...
		// y descripción
		des=creaObjProp('div', {className:'des_bsc', id:'des_'+idBSP})
		var espacio=creaT(' ')
		bsp.appendChild(cmd); bsp.appendChild(txt); bsp.appendChild(espacio); bsp.appendChild(des)
		}
	this.txts=[txt]
	...
	if (this.claseCSS) {
		...
		if (this.colorFondo)
			txt.style.background= this.colorFondo
		if (this.colorFrente) { 
			txt.style.color= this.colorFrente
...
ControlDESP.prototype.propsPorDefecto={ancho:3500, alto:300, izq:300, caption:'nivelVacio'}
...
function mueveRubberBand(event){
	try 	{
		if (disenhador!=null && disenhador.modoDisenho)

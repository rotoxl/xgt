...
Tramite.prototype.pinta=function(cont, esCopia, id, i, j, padre){
	...
	var contTram=creaObjProp('div', {className:'contTram', 'style.left':i*Tramite.prototype.ancho*1.6  - cont.childNodes.length*Tramite.prototype.ancho})
...
MapaTramitacion.prototype.centrar=function(nombre){
	...
	var yaExiste=control('foco')
	if (yaExiste) yaExiste.parentNode.removeChild(yaExiste)
	
	var offset=tram.offset()
	var foco=creaObjProp('div', {id:'foco',
					className:'foco', 
					'style.left':offset.left,
					'style.top':offset.top,
					'style.width':tram.outerWidth(),
					'style.height':tram.outerHeight()
					})
				
	this.contenedor.appendChild(foco)
	
	foco.scrollIntoView(false)
...
MapaTramitacion.prototype.pintaLeyenda=function() {
	...
	var literalCamino=creaObjProp('a', {className:'botonera texto gris', 'texto':'Camino'})
	
	var self=this
	this.ctlListaCaminos=creaObjProp('select', {id:'listaCaminos', onchange:function(){self.clickListaCaminos()} })
	this.cargaListaCaminos(this)
	
	var btnNuevoCamino=creaObjProp('a', {className:'botonera', onclick:fnNuevoCamino(), title:'Nuevo camino', id:'nuevoCamino'} )
	var literalTramite=creaObjProp('a', {className:'botonera texto gris', 'texto':'Trámite'})
	var btnNuevoTramite=creaObjProp('a', {className:'botonera', onclick:fnNuevoTramite(), title:'Nuevo trámite', id:'nuevoTramite'} )	
	
	var btnTam1=creaObjProp('a', {className:'botonera', id:'tamanho1', onclick:fnCambiaEscala(.25), title:'Tamaño pequeño'})
	var btnTam2=creaObjProp('a', {className:'botonera', id:'tamanho2', onclick:fnCambiaEscala(.5), title:'Tamaño mediano'})
	var btnTam3=creaObjProp('a', {className:'botonera', id:'tamanho3', onclick:fnCambiaEscala(1), title:'Tamaño grande'})
...
MapaTramitacion.prototype.setListaCaminos=function(datos){
	this.caminos=datos
	for (var id in this.caminos){
		var cam=this.caminos[id]
		var opt=creaObjProp('option', {	value:cam.CD, 
								text:cam.CD + ' - ' + cam.DS + ' ('+(cam.tt?'reglada, ':'no reglada, ')+cam.numTramites+' trámites)'})
		this.ctlListaCaminos.appendChild(opt)
		}
	if (this.idCamino==null) {
		var opt=creaObjProp('option', {value:null, text:''})
		this.ctlListaCaminos.appendChild(opt)
		this.ctlListaCaminos.value=null
		}
	else
		this.ctlListaCaminos.value=this.idCamino
	}
MapaTramitacion.prototype.clickListaCaminos=function(){//(cam.CD, aplicacion)
...
MapaTramitacion.prototype.pintaRelacion=function(id, n, m)  {
	...
	if (origen.top == destino.top ) 
		raya=creaObjProp('div', {id:id, className:'raya rayah', 'style.left':origen.left, 'style.top':origen.top+y, 'style.width':destino.left-origen.left, 'style.height':1} )
	else {
		//___
		//  |
		//  |___
		...
		raya=creaObjProp('div', {id:id, className:'raya rayav', 'style.left':origen.left+x, 'style.top':origen.top+y, 'style.width':destino.left-origen.left-x, 'style.height':destino.top-origen.top})
		}

...
if (i<-1) return "menos "+get(-i, acabaEn);
	...
	public static String get(FechaGotta fg, String formato){
		...
		if (formato.equalsIgnoreCase("DMAL")){//trece de febrero de dos mil uno
			...
			ret=NumeroTexto.get(fg.get(Calendar.DATE), true)+formatoFecha.format(fg.getTime())+NumeroTexto.get(fg.get(Calendar.YEAR),true);
			}
			...
			ret=NumeroTexto.get(fg.get(Calendar.DATE),true)+formatoFecha.format(fg.getTime());
			}

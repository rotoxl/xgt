...
function FormularioConexiones(idxBloqueo){
	...
	this.lista=null
	this.botones=[	
			creaObj('button', 'cmdgt', null, 'Aceptar', function(){editorConexiones.aceptaCatalogos()} ) , 
			creaObj('button', 'cmdgt', null, 'Cancelar', function(){ editorConexiones.ocultaFormulario() })
			]
}
...
FormularioConexiones.prototype.muestraListaCatalogos=function(){
	this.lvw.rellenaLista(['Fuente de datos Tomcat', 'Catálogo', 'Esquema', 'SGBDR'], ['conexion', 'catalogo','esquema','tipoBD'], this.lista, 'id', null, null, true)
}
...
function FormularioEditorDiccionario(idxBloqueo){
	...
	this.porcentajeAltoFrm=100
	this.porcentajeAnchoFrm=100
...
FormularioEditorDiccionario.prototype.onResize=function(dep){
	...
	var w=control(dep.idx).offsetWidth
	var h=control(dep.idx).clientHeight-dep.altoBotonera-dep.altoCabecera-dep.altoPie
	
						// sSplId,  	  X,  Y,  W,   H,  Horz ,  BarW, BarPos, BarLim,    BarEn, BorW, oSplChild1, oSplChild2				       
	var splitterH = new xSplitter('splitterDic',  0, -5, w, h, true,   7,  w/3,    h/8,     true, 0)
	try{
...
FormularioEditorDiccionario.prototype.cancelar=function(){
	window.location='./login.jsp?aplicacion='+aplicacion
}

...
function muestraifr(){
	_ifrVisible=true
	var fn=function(){
		if (!_ifrVisible) return
		var ifr=control(__panelSQL__)
...
FormularioEditorNiveles.prototype.anhadeAsistentes=function(ul, div ) {
	...
	for (var k in asistentes) {
		...
		var enlace
		if (ml.ds.startsWith('-'))
			enlace=creaObjProp('a', {className:'menuML separador', texto:ml.ds.substring(1)})
		else
			enlace=creaObjProp('a', {onclick:fnEnchufaAsistente(ml.id), className:'menuML '+ml.css, texto:ml.ds, hijo:creaObjProp('span', {className:'dsExtendida', texto:ml.dsExtendida})})
...
FormularioEditorNiveles.prototype.pintaCuerpo=function() {
	...
	splitter.appendChild( creaObjProp('div', {className:'section clsPane', 	id:__panelSQL__}) )
...
FormularioEditorNiveles.prototype.asistentes=function(){
	...
	var asistentes= {	'-sep00':{ds:'-Árboles y listas'}
					...
					,'-sep03':{ds:'-Permisos sobre trámites y firma electrónica de trámites y documentos'}
...
function FormularioParametros(idxBloqueo){
	...
	var cmdAceptar=creaObj('button', 'cmdgt', null, 'Aceptar', function(){editorParametros.acepta()})
	var cmdCancelar=creaObj('button', 'cmdgt', null, 'Cancelar', function(){ editorParametros.cancela() }) 
...
FormularioParametros.prototype.pintaCuerpo=function() {
	...
	var ventana=creaObjProp('div',{className:'cuerpoParametros', hijos:[btn, advertencia]})

...
var salvaDato=function(nombre, propiedad, valor){
  if (nombre!="splitter") {
	  _salvaDato(nombre, propiedad, valor)
	  }
}*/
/*
...
//redefinimos la función 'tareasPostCarga', que se llama al final del arranque de nuestra aplicación:
Explorador.prototype.tareasPostCarga=function(){ 
   //obtenemos la versión de internet explorer (-1 significa que no es internet explorer)
   var versionIE=versionInternetExplorer()       
   if (versionIE > 0) {
        //añadimos una clase especial a los elementos conflictivos: esta clase será ie6, ie7, ie8 en función de la versión del navegador
        ponEstilo( control('botoneraIzq'), 'ie')
        ponEstilo( control('botoneraAdmin'), 'ie')
        ponEstilo( control('botoneraDra'), 'ie')      
        }
}
...
/* Nacho
Vista "Simulacion de presupuestos", lista de centros de costes
*/
Lista.prototype.tareasPostCarga=function(){
	if (this['tareasPostCarga_'+this.idControlDestino])
	this['tareasPostCarga_'+this.idControlDestino]()
	}
Lista.prototype.tareasPostCarga_control20318=function(){
$(this.tbody).find('tr.fila').click(function(){
desSeleccionarTodos(this.parentNode)
seleccionarFila(this)
lanzaTramite(1000, 'CargaCentroCoste')
})
}
	...
	for (var i=0; i<datos.length; i++) {
		...
		if (dato != null && typeof dato!='function' && !dato.esBorrado){
			...
			if (onContextMenu!=null){
					tr.oncontextmenu=function(event){seleccionarFila(this,null,true)
	...
	for (var j=primeraFila; j<datosFilas.length; j++) {
		...
		if (tipoLista=='tp' || tipoLista=='te' || tipoLista=='tf' || tipoLista=='tl' || tipoLista=='lt' || tipoLista=='pb') {
			...
			tr.oncontextmenu=function (event) {
				seleccionarFila(this,null,true)
				muestraMenu(event, null, key);
				return false;
				}
			}
		else if (tipoLista=='lvw' )
			tr.oncontextmenu=function(event){muestraMenu(event, 'lvw', key); return false;}
...
Lista.prototype.ponDivMaxElementos=function(datosFilas, datosLVW){
	if ( datosLVW && !datosLVW.datos.maxElementosAlcanzado ) {
		...
		if (maxElementosAnterior!=null) //reutilizamos el anterior.Sólo tenemos que cambiar el enlace 'cargar otros 25'
			...
			div.className='maxElementos'
			div.id='maxElementos'+this.numControl
			
			var enlace1=creaA('#',
						 '',
						 this.fnCargar(this.numControl, datosFilas.length+maxElementos),
						 null,
						 null) // creaA(href, titulo, onClick, clase, target)
			enlace1.id='maxElementos'+this.numControl+'_enlace1'
			var enlace2=creaA('#',
						 '',
						 this.fnCargar(this.numControl, -1),
						 null,
						 null) // creaA(href, titulo, onClick, clase, target)
			enlace1.appendChild( creaTexto('Cargar otras '+maxElementos + ' filas') )
			enlace2.appendChild( creaTexto('Cargar todas') )
...
LVW.prototype.fnCargar=function(numControl, cuantos){
	return function(){ 	var lista=exp.mdActivo().getControl('control'+numControl).lvw
...
function fnVacia(){}
function ponEstilo(fila, estilo, sinQuitar){// si estilo contiene _, se supone que es una opción excluyente de las de la misma raíz
	var trozos=estilo.split('_')
	var raiz=trozos[0]; var resto=trozos[1]
	var excluyente= resto!=null
	if (sinQuitar) 
		excluyente=false
	
	var espacio=' '
	var temp= fila.className.split(espacio)
	if (excluyente){
		for (var i=0; i<temp.length; i++){
			if ( temp[i].startsWith(raiz) )
				temp.remove( temp[i] )
			}
		}
	
	if (!temp.contains(estilo) )
		temp.push(estilo)
	fila.className=temp.join(espacio)
}
...
function tieneEstilo(fila, estilo){
	return (' '+fila.className+' ').contains(' '+estilo+' ')
}
////////////////////////////////

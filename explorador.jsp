...
<body id="explorador"  class="<%=apli%> <%=usr.getIdioma()%>" onload="init_gotta()" style="overflow:hidden;">
	<div class="header panelSup">
		...
		<div id="botoneraAdmin" class="nav botonera jquerycssmenu" tabindex=0> 
			...
			<li>
				<a tabindex="-1" rel="nofollow" href="javascript:activarDisenho()" title="[Dise&ntilde;o inactivo] Activar dise&ntilde;o" class="botonera" id="btnDisenhador"><span>Dise&ntilde;o</span>&nbsp;</a>
			</li>
			<%}%>
			<%if (usr.permisoDepuracion) {%>
				<li>
					<a tabindex="-1" rel="nofollow" href="javascript:activarDepuracion()" title="[Depuraci&oacute;n inactiva] Activar depuraci&oacute;n" class="botonera" id="btnDepurador"><span>Depuraci&oacute;n</span>&nbsp;</a>
				</li>
				<li>
					<a tabindex="-1" href="mapa.jsp?aplicacion=<%=refApli%>" target="_blank" rel="noreferrer" title="Mapa de tramitación" class="botonera" id="btnConstructor"><span>Mapa de tramitación</span>&nbsp;</a>
				</li>
				<li>
			...
			<%if (usr.permisoMonitorSQL) {%>
				<li>
					<a tabindex="-1" rel="nofollow" href="monitor.jsp?aplicacion=<%=refApli%>" target="_blank" rel="noreferrer" title="Monitor de la aplicación" class="botonera" id="btnMonitorSQL"><span>Monitor</span>&nbsp;</a>
				</li>
			<%}%>
			<%if (usr.permisoRearrancar) {%>
				<li>
					<a tabindex="-1" rel="nofollow" href="recarga.jsp?aplicacion=<%=refApli%>" target="_blank" rel="noreferrer" title="Recargar aplicaci&oacute;n" class="botonera" id="btnRecargarAplicacion"><span>Recargar</span>&nbsp;</a>
				</li>
				<li>
					<span class="botonera separador" >&nbsp;</span>
				</li>
		...
		<div id="botoneraDra" class="nav botonera jquerycssmenu" tabIndex=0>
			<ul>
				<li>
					<a tabindex="-1" rel="nofollow" id="btnRefrescar" class="botonera imagen texto" title="Actualizar (F5)" href="?aplicacion=<%=refApli%>&amp;refrescoCompleto=1" target="_parent"><span>Actualizar</span></a> 
				</li><li>
				<span class="botonera separador" >&nbsp;</span>
				</li><li>
					<a tabindex="-1" rel="nofollow" id="btnVersionParaImprimir" class="botonera imagen texto" href="imprimir.jsp?aplicacion=<%=refApli%>" target="_blank" rel="noreferrer" title="Versión para imprimir"><span>Versión para imprimir</span></a>
				</li><li>
					<a id="btnNuevaVentana" class="botonera imagen texto" tabindex="-1" href="?aplicacion=<%=refApli%>+" title="Nueva ventana: inicia una nueva sesión con filtros de búsqueda independientes" target="_blank" rel="noreferrer"><span>Nueva ventana</span></a>
				</li>
				<%if (!usr.esUsuarioAnonimo()) {%>
					<li>
						<span class="botonera separador" >&nbsp;</span>
					</li><li>
					<a tabindex="-1" rel="nofollow" id="btnCambiarContrasenha" class="botonera imagen texto" title="<%=usr.getDsUsuario()%>" href="javascript:lanzaTramite( '', 'gtUsuario')" title="Mi cuenta de usuario"><span>Mi cuenta</span></a>
						</li>
				<%}%>
				<li>
				<span class="botonera separador" >&nbsp;</span>
				</li><li>
				<a tabindex="-1" rel="nofollow" id="btnSalir" class="botonera imagen texto" href="salir?aplicacion=<%=refApli%>" target="_parent" title="Salir"><span>Salir</span></a>
				</li>
			</ul>
		</div>
	</div>

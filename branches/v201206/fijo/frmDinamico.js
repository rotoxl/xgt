...
Frm.prototype.toDOM=function() {
	...
	var ventana=creaObjProp('div', {className:'ventana formulario', 'id':this.idx, 'style.left':izq, 'style.width':this.ancho})	
...
Frm.prototype.pintaCabecera= function (){
	this.icono=creaObj('div', 'icono')
	var tit=this.titulo || this.textoTitulo
	this.pTitulo=creaObj('p', null, null, tit )
...
Formulario.prototype._pintaContenedorControles=function(cuerpofrm, altoCuerpo){
	//cuerpofrm.style.height=altoCuerpo + 'px'
	var tabla=creaObj('table')
...
Formulario.prototype.toDOM= function (){
	...
	this.ventana.parentNode.appendChild( this.spanGloboAyuda )
}
...
Formulario.prototype._pintaBSC = function ( ctl ){
	var div=creaObj('div', 'bsc', ctl.id)
	if(ctl.colsBloqueadas)
		ponEstilo(div, 'colsBloqueadas')
	ponEstilo(div, ctl.bloqueado? 'Xno':'Xsi')
	
	var self=this
	var fnAbreBuscador=function(){
		abreVentanaBuscador(ctl, null, self); 
		return false;
		}
	var fnBuscar=function(event){
		b.buscarPorCodigo(false)
		}
	var fnEsperarYBuscar=function(event){
		b.esperarYBuscarPorCodigo(false)
		}
		
	//1º el botón de buscar
	if (!ctl.bloqueado)
		div.appendChild( this.creaA('cmd_bsp', '#', fnAbreBuscador, '[Click aquí o F2 para buscar]', espacioDuro4) )
	...
	//3º la descripción
	ctl.des=creaObj('div', 'des_bsc', 'des_'+ctl.id)
	div.appendChild( ctl.des )
...
Formulario.prototype._pintaTXT = function ( ctl, onblur, onkeyup, onkeypress, esBsc, grupoActivo){	
	...
	if (ctl.tipo=='lvwFecha'){
		var div=creaObj('div', 'calendario')

		var a=generaBotonCalendario(ctl.bloqueado, ctl.id)
...
function generaBotonCalendario(bloqueado, id){
	if (!bloqueado){
		var a=creaObjProp('a', {className:'cmd_calendar',
						'href':'#',
						onclick:fnOpenCalendar(id),
						texto:espacioDuro4,
						'tabIndex':-1})
		return a
		}
}
function fnOpenCalendar(id){
	return function(){new Calendario( arguments[0], id, control(id).value )} 
	}
Formulario.prototype.pintaGloboAyuda = function (ctl){
	...
	if (dsExtendida){
		...
		var self=this
		$(sp).hover(
			function(){self.montaAyuda(ctl)},
			function(){self.ocultaAyuda(ctl)}
			)
...
Formulario.prototype.pintaFila = function (tabla, etiqueta, obligatorio, casilla, idCasilla, idCtl, ctl) {
	...
	if (casilla==null) {
		...
		var tdError=creaObj('td', null, 'error'+idCtl)
...
Formulario.prototype.pintaFilaVacia=function(tbody) {
	...
	tr.appendChild( creaObj('td', 'errorjsOFF') )
...
Formulario.prototype._enviaArchivos=function(){
	...
	for (var i=0; i<this.controles.length; i++) {
		...
		if (ctl.tipo == 'lvwFile') {
			...
			if (ctlFile.value!=''){
				if (!hayArchivos)
					this.idxBloqueoSubir=ponNuevoBloqueo(true, 'subiendo')
	...
	if (hayArchivos){
		var self=this
		AIM.submit(frmaim, {'onStart' : function(){self.inicioSubirFicheros()}, 'onComplete': function(html){self.finSubirFicheros(html)}})
		frmaim.submit()
		return true
		}
	else  {
...
Formulario.prototype.finSubirFicheros=function(texto){
	...
	if (texto.contains('error')){
		if (texto.startsWith('<pre>'))
			texto=texto.substring('<pre>'.length, texto.length-'</pre>'.length)
		
		var ret=JSONparse(texto)
...
Formulario.prototype.envia=function(reintentos){
	...
	if (this.valida()) {
		...
		window.setTimeout(function(){self.ocultaAyuda(self)},6000)
		}
}
...
////////////////////////////////////////////////////////////
//~ function FormularioEnLista(datos, idxBloqueo, notificarCierre, lvw){
	//~ Formulario.call(this, datos, idxBloqueo, notificarCierre)
	//~ this.lista=lvw
	//~ }
//~ FormularioEnLista.prototype=new Formulario
//~ FormularioEnLista.prototype.toDOM= function (){
	//~ this.lista.lvw.montaFrm(this)
	//~ quitaBloqueo(this.idxBloqueo)
//~ }
//~ FormularioEnLista.prototype.ocultaFormulario=function(quitarBloqueo){
	//~ this.lista.lvw.desmontaFrm(this)
//~ }
////////////////////////////////////////////////////////////
...
Formulario.prototype.montaAyuda=function(casilla, titulo, texto){
	if (! casilla.ventanaAyuda){
		var ventanaAyuda=creaObjProp('div', {className:'globoAyuda', 'style.display':'none'})
		casilla.spanGloboAyuda.appendChild(ventanaAyuda)
		
		if (!titulo) 	ventanaAyuda.appendChild( creaObjProp('span', {className:'flecha', 'texto':espacioDuro4}))
		ventanaAyuda.appendChild( creaObjProp('span', {className:'titulo', 'texto':titulo || casilla.ds}) )
		ventanaAyuda.appendChild( creaObjProp('span', {className:'texto', 'textos':texto || casilla.globoAyuda}) )
		
		casilla.ventanaAyuda=ventanaAyuda
		}
	var $ventanaAyuda=$(casilla.ventanaAyuda)
	var offset=$(casilla.spanGloboAyuda).offset()
	$ventanaAyuda.css({left:offset.left+'px', top:offset.top+'px'})		
	$ventanaAyuda.fadeIn(jquerycssmenu.fadesettings.overduration)
}
Formulario.prototype.ocultaAyuda=function(casilla){
	$(casilla.ventanaAyuda).fadeOut(jquerycssmenu.fadesettings.overduration); 
}
////////////////////////////////////////////////////////////
...
FormularioBSC.prototype.pintaCuerpo=function(altoCuerpo) {
	...
	var cuerpofrm=creaObj('div','cuerpoBS')
	...
	var div1=creaObjProp('div', {className:'cuadroBusqueda', texto:FormularioBSC.prototype.textoBuscar})

	this.texto=creaObjProp('input', {type:'search', className:'lvw', maxlength:80,  onkeyup:fn})
		div1.appendChild(this.texto)

	this.throbber=creaImg(icoGotta+'throbber.gif', 'Cargando...', 'throbber', 'Cargando...', this.idxThrobber) //className, src, title, alt, id->src, alt, clase, titulo,id
		this.throbber.style.visibility='hidden'
		div1.appendChild(this.throbber)
	...
	this.botonAltaDinamica=creaObj('button', 'cmdgt altaDinamica', 'altaDinamica', 'Nuevo')
		...
		div1.appendChild(this.botonAltaDinamica)
...
FormularioBSM.prototype.pintaCuerpo=function(altoCuerpo) {
	...
	this.tabla.parentNode.className='tablaBusquedaMultiple'
...
FormularioBSM.prototype.rellenaTabla=function(datos){
	...
	$(this.tabla).parents('div.ventana').addClass('formulario_bsm')
...
function retornoTramite(respuesta, w, iInicial){
	...
	if ( respuesta.tipo=='frm') {
		...
		if (respuesta.clase=='progreso'){
			var p=$('#'+respuesta.idxBloqueo + ' div.cargandoIndicador')
			
			if (p.length>0)
				p.text(espacioDuro+respuesta.texto)
			else
				ponBloqueo(respuesta.idxBloqueo, true, respuesta.texto)
...
Buscador.prototype.ponThrobber=function(){
	if (this.cmdbsp) ponEstilo(this.cmdbsp, 'buscando')
	}
Buscador.prototype.quitaThrobber=function(){
	if (this.cmdbsp) quitaEstilo(this.cmdbsp, 'buscando')
	}
...
Buscador.prototype.sustituyeDescripcion=function(texto) {
	this.ctl.des.innerHTML=noNulo(texto)
	}

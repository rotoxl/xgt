...
function _ocultaMenu(contextMenu) {
	if (seguirMostrando)
		return;
	if (contextMenu != null)
...
function completaRutaImg(icono, esParaBackgroundImage){
	if ( icono && icono!='' ) {
		var exp=exp || {'icoGotta':icoGotta, 'icoWeb':icoWeb}
		var ruta
		icono+=''
...
var listaActiva=null // el número del lvw o arb
function muestraMenu(event, cual, parametros)  {//donde parametros = clave, camino, tramite, fechaPendiente
	...
	if (temp[0])
		listaActiva = temp[0].id  /* anotamos la lista que está activa */
	var contextMenu = control(cual)
	if (contextMenu==null)
		contextMenu=montaMenuLVW(cual)
	
	colocarMenu(contextMenu, event)
	return false
}
function colocarMenu(contextMenu, event){
	contextMenu.style.left = event.clientX +'px' 
	contextMenu.style.top = event.clientY  +'px'
	contextMenu.style.display='block'
	contextMenu.style.visibility='visible'
	var wHeight= window.innerHeight || document.documentElement.offsetHeight
	if(contextMenu.offsetHeight+contextMenu.offsetTop> wHeight)
		contextMenu.style.top= wHeight-contextMenu.offsetHeight +'px'
}
function creaM(id, tipo, clase, texto, onclick){
...
function montaMenuLVW(cual,tipoOperacion,id) {//tp, te, tf, tl, lt
	ocultaMenus()
	...
	menu.onmouseover=function(){sigueMostrandoMenu()}
	menu.onmouseout=function(){ocultaMenu(this.id)}
	
	var todos=		creaM('btnSeleccionarTodos', 'div', 'menuitems', Explorador.prototype.textoSeleccionarTodos, function(){seleccionarTodos(); })
	var invertir= 	creaM('btnInvertirSeleccion', 'div', 'menuitems', Explorador.prototype.textoInvertirSeleccion,  function(){ invertirSeleccion()})
	var sep=		creaObjProp('HR', {'style.width':'90%', size:1})
	var ejecutar=	creaM('btnEjecuta', 'div', 'menuitems', Explorador.prototype.textoEjecutar, function(){ejecuta()})
	var mRevivir=	creaM('btnRevivir', 'div', 'menuitems', Explorador.prototype.textoRevivir, function(){revivir()})
	var mReemitir=	creaM('btnReemitir', 'div', 'menuitems', Explorador.prototype.textoReemitir, function(){reemitir()})
	...
	if (cual=='lvw') {//menú para las listas normales y corrientes, nada de tramitación
		...
		menu.appendChild( invertir )
		if (cual=='tf') {
			var mVerDatos	=creaM('btnVerDatosOriginales', 	'div', 'menuitems', Explorador.prototype.textoVerInformacionFirmada, datosFirmados)
			var mVerFirma	=creaM('btnVerFirmas', 		'div', 'menuitems', Explorador.prototype.textoVerInformacionFirma, jsonInfoFirma)
			var mValidarFirma=creaM('btnValidarFirma', 		'div', 'menuitems', Explorador.prototype.textoValidarFirma, jsonValidaFirma)
		...
		var pendiente=creaM('btnDejarPendiente', 'div', 'menuitems', Explorador.prototype.textoDejarPendiente, function(){ejecuta('pendiente')})
		...
		var ejecutarLote=creaM('btnEjecutarLote', 'div', 'menuitems', Explorador.prototype.textoEjecutar, function(){ejecuta('lotes')})
		...
		var borrar = creaM('btnBorrar','div','menuitems','Borrar',function(){editorDiccionario.borrarVirtual(tipoOperacion, id)})
		var alta = creaM('btnAlta','div','menuitems','Alta',function(){editorDiccionario.nuevoVirtual(tipoOperacion, id)})
		...
		var abrirTablas=	  creaM('btnDepuradorAbrirTablas', 'div', 'menuitems', 'Abrir todas', function(){depuradorActivo.abrirTablas(); })
		var abrirTablasConFilas=creaM('btnDepuradorAbrirTablasConFilas', 'div', 'menuitems', 'Abrir sólo las que tienen filas', function(){depuradorActivo.abrirTablasConFilas(); })
		var cerrarTablas=creaM('btnDepuradorCerrarTablas', 'div', 'menuitems', 'Cerrar todas', function(){depuradorActivo.cerrarTablas(); })
		
		var ordenarA=creaM('btnDepuradorOrdenarTablasAZ', 'div', 'menuitems', 'Ordenar alfabéticamente', function(){depuradorActivo.ordenarTablasAlfab(); })
		var ordenarB=creaM('btnDepuradorOrdenarTablasBloques', 'div', 'menuitems', 'Ordenar según el tipo (tabla base y de trámites primero, tablas con datos después)', function(){depuradorActivo.ordenarTablasConDatos(); })
...
function _seleccionarTodos(todos) {
	...
	if (todos){//seleccionar todos
		$('#'+listaActiva+' table .fila').addClass('seleccionada')
		$('#'+listaActiva+' input:checkbox:not(:checked)').attr('checked', true)
		}
	else {//invertir selección
		$('#'+listaActiva+' table .fila').toggleClass('seleccionada')
		$('#'+listaActiva+' input:checkbox').each( function(){
...
function jsonFirmas(accion,msg){
	ocultaMenus()
	var md=exp.mdActivo()
	var mm=md.getControl(listaActiva)

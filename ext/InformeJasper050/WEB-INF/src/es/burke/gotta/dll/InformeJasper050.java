...
public class InformeJasper050 extends DLLGotta {
	//ojo: requiere jasperreports-0.6.8_ORA.jar y poi-2.5.1-final-20040804.jar (para exportar a excel)
	{
	...
	private static final long serialVersionUID = -4031305330216189893L;
	...
	private void trucoClassLoader() throws ErrorGotta {
		URL urls [] = {};
		JarFileLoader cl = new JarFileLoader (urls);
		String r=this.mMotor.getApli().getRutaFisicaAplicacion()+"WEB-INF/lib/";
		
		cargaJar(cl, r+"jasperreports_antiguo/jasperreports-0.6.8_ORA.jar");
		cargaJar(cl, r+"jasperreports_antiguo/poi-2.5.1-final-20040804.jar");
		}
	private void cargaJar(JarFileLoader cl, String r) throws ErrorGotta{
		try {
			cl.addFile ( Util.arreglaRutaFisica(r) );
			} 
		catch (MalformedURLException e) {
			throw new ErrorGotta("No se han podido cargar ");
			}
		}
	@Override
...
//				exportoA="odt";
//			else 
			if ( nombreInforme.toLowerCase().endsWith(".xls") )
				exportoA="xls";
			else
				exportoA="pdf";
			...
			for (JRParameter jp : jrparams) {
				if (! jp.isSystemDefined() && !jp.getValueClass().getName().equals(java.sql.ResultSet.class.getCanonicalName()))  { 
					...
					if (nombreParamMinus.startsWith("subreport_rutaimg"))
			    		parameters.put( nombreRealParam,  apañaRuta(PoolAplicaciones.dirInstalacion+mMotor.getApli().getDatoConfig("icoWeb") ) );
					else if (nombreParamMinus.startsWith("subreport_ruta"))
			    		parameters.put( nombreRealParam, dirRutaVerificada());
			    	else {
			    		if (acc.equalsIgnoreCase("EmitirDocumento") || acc.equalsIgnoreCase("EmitirGuardar")) {
				    		Variable v=null;
				    		...
				    		if (porNombre && Util.en(nombreParamMinus,  letrasVálidas))
				    			...
				    			if (v==null) //nombres de variable largos @expediente
					    			v=mMotor.lote.getVariable(nombreParamMinus);
				    			}
				    		i++;
				    		...
				    		if (valor==null){
								...
								String clase = jp.getValueClass().getName().toLowerCase();
			    		...
			    		else if (acc.equalsIgnoreCase("EmitirDocumentoParamsDefecto")) {
			    			String expresion=jp.getDefaultValueExpression().getText();
			    			valor = evaluacionMuyCutre(expresion, jp.getValueClass().getName());
			    			}
	...
	public void verifica(Motor motor, String accion, String nombreInforme) throws ErrorGotta {
		String dirlib=Util.arreglaRutaFisica(motor.getApli().getRutaFisicaAplicacion()+"/WEB-INF/lib/");
		String dirclasses=Util.arreglaRutaFisica(motor.getApli().getRutaFisicaAplicacion()+"/WEB-INF/classes/");
		System.setProperty("jasper.reports.compile.class.path",dirlib+System.getProperty("file.separator")+"jasperreports-0.5.0.jar" + System.getProperty("path.separator")+ dirclasses);
				
		this.mMotor=motor;
		this.usr=motor.usuario;
		this.apli = motor.getApli();
		rptWeb=apli.getDatoConfig("rptWeb");
		if (rptWeb==null)
...
//			else 
			if (this.exportoA.equalsIgnoreCase("xls") )
...
//				exporter = new JROdtExporter();
			...
			usr.añadeMSG("Error generando el informe "+this.nombre+" ("+this.rutaVerificada+") con los params "+Util.toString(this.parameters)+": "+e.getMessage() + ": " +e.getStackTrace() , TipoMensajeGotta.alerta);
			throw new ErrorGenerandoProducto(e);
			}
		finally {
		...
		if (this.rutaGuardar==null){
			...
			ServletOutputStream salida = res.getOutputStream();
			InputStream fis=new FileInputStream(rutaDest);
...
//						analiza( _hazArray( ( (JRElementGroup)ch ).getElements()), ret );
	...
	private ArrayList<Object> bandasInforme() {
		...
		ret.add( this.jasperReport.getDetail());
